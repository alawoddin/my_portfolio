...
<div class="card my-3">
  ...
  <div class="card-body">
    <form action="{{ route('update.about') }}" method="POST" enctype="multipart/form-data" class="row g-3">
      @csrf
      ...
      <div class="col-md-6">
        ...
        <input type="text" id="home_title" name="title" class="form-control"
               value="{{ old('title', $abouts->title) }}" required>
      </div>
  ...
  <textarea
    ...
    class="form-control"
    required
  >{{ old('information', $abouts->information ) }}</textarea>
      ...
      <div class="col-md-6">
        ...
        <img id="home_photo_preview"
             src="{{ $abouts->photo_url ?? (isset($abouts->photo) ? $abouts->photo : url('upload/no_image.jpg')) }}"
             alt="current image"
